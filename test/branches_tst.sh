...
function check_n_commits_by_msg {
  ...
  if [ "$N" = "0" ]; then
    echo "Commit '$commit_msg' not found or doesn't exist in branch $BRANCH as expected."
    exit 1
  fi
...
function test_tree_structure {
  ...
  for C_PARENT in $C_PARENTS; do
    C_PARENT_MSG=$(git log --format="%B" -n 1 $C_PARENT)
    if [ "$C_PARENT_MSG" = "$parent_msg" ] || [ "$C_PARENT_MSG" = "$parent2_msg" ]; then
      break
    else
      echo "The parent of '$commit_msg' is '$C_PARENT_MSG', but expected $parent_msg or $parent2_msg"
      exit 1
    fi
  done

  echo -e "'$commit_msg' is defined correctly!\n"
}
...
# Check and test tag v1.0.2
if check_tag_exists "v1.0.2"; then
...
# Check and test tag v1.0.3
if check_tag_exists "v1.0.3"; then
